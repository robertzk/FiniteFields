...
class FiniteField
  ...
  def multiplication_table(html_file)
    File.open(html_file, 'w') do |f|
      ffp_string = default_poly ? '' :
        ", FiniteFieldPolynomial.new(#{prime}, #{polynomial.coefficients.to_s[1..-2]})"
      finite_field_string = "FiniteField.new(#{prime}, #{exponent}#{ffp_string})"
      f << <<-end_head
 ...
 #{finite_field_string}.multiplication_table('#{html_file}')</pre></h3>
      end_head
